...
@Dependent
public class FlowThumbnailProvider extends BindableShapeSetThumbProvider {

    @Inject
    public FlowThumbnailProvider(final DefinitionManager definitionManager) {
    ...
    protected boolean thumbFor(final Class<?> clazz) {
        return isSameClass(clazz,
                           FlowDefinitionSet.class);
    }

    @Override
    public SafeUri getThumbnailUri() {
        return FlowImageResources.INSTANCE.bpmnSetThumb().getSafeUri();
    }
}
