 ...
 */
@Dependent
public class MultipleSubFormFieldPropertiesGenerator extends FieldPropertiesGenerator<MultipleSubFormFieldDefinition> {
    @Inject
    ...
    public void loadFieldProperties( final FieldDefinitionPropertiesModal modal ) {
        finderServiceCaller.call(new RemoteCallback<List<SubFormData>>() {
            ...
            public void callback(List<SubFormData> response) {
                ...
                MultipleSubFormFieldPropertiesGenerator.super.loadFieldProperties(modal);
            }
        }).getAvailableMultipleSubFormsByType(field.getStandaloneClassName(), draggableFieldComponent.getFormPath());
    }
    ...
    protected void renderHTMLContent( String template ) {
        ((MultipleSubFormFieldRenderer)draggableFieldComponent.getFieldRenderer()).setHtmlContent( template );
    }
